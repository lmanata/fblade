...
public class Play extends GameState{
	...
	public Play(GameStateManager gsm) {
		...
		for(int i = 0;i<=23;i++) newSegment(segmentY);
	...
	public void newSegment(float y){
		...
		border.newChain(
				new Vector2(0f,segmentY),
				new Vector2(0f,segmentY+0.32f),
				zawarudo
					);
		
		border.newChain(
					new Vector2(3.04f,segmentY),
					new Vector2(3.04f,segmentY+0.32f),
					zawarudo
						);
	...
	public void handleInput(){
		...
		if(Gdx.input.isKeyPressed(Input.Keys.LEFT)){
			...
			switch(difficulty){
			case 1 : player.getBody().setLinearVelocity(-6f,zawarudo.getGravity().y);break;
			case 2 : player.getBody().setLinearVelocity(-8f,zawarudo.getGravity().y);break;
			case 3 : player.getBody().setLinearVelocity(-9f,zawarudo.getGravity().y);break;
			case 4 : player.getBody().setLinearVelocity(-12f,zawarudo.getGravity().y);break;
			case 5 : player.getBody().setLinearVelocity(-15f,zawarudo.getGravity().y);break;
			}
		}
	...
	public void update(float dt){
		...
		if(score > 7) difficulty = 2;
		if(score > 14) difficulty = 3;
		if(score > 20) difficulty = 4;
		if(score > 30) difficulty = 5;
		
		if(collisions.getCollapse()){
			scorehandler.print((Float.toString(score)).substring(0, ((Float.toString(score)).length()<4)?(Float.toString(score)).length():4)+";");
		...
		newRandom();
		for(Vector2 vec : toadd) newWall(vec.x,vec.y);
		toadd.clear();
	...
	public void render(){
		...
		switch(difficulty){
		case 1 : zawarudo.setGravity(new Vector2(0f,3f));break;
		case 2 : zawarudo.setGravity(new Vector2(0f,5f));break;
		case 3 : zawarudo.setGravity(new Vector2(0f,7f));break;
		case 4 : zawarudo.setGravity(new Vector2(0f,10f));break;
		case 5 : zawarudo.setGravity(new Vector2(0f,13f));break;
		}
		...
		if(segments.size() > 0) lasts = segments.get(segments.size()-1);
		
		if(!(cam.frustum.pointInFrustum(lasts.nextposition)) && (lasts.nextposition.y - player.getPosition().y*10) < lasts.nextposition.y ){
			newSegment(segmentY);
		}
		
		for(Segments segment: segments){
			if((!cam.frustum.pointInFrustum(segment.position)) && segment.body.getPosition().y+5 < player.getPosition().y){
				bodylist.add(segment.body);
				segsRem.add(segment);
			}else{segment.render(sb);}
		}
		...
		layout.setText(Font, "Score:"+(Float.toString(score)).substring(0, ((Float.toString(score)).length()<4)?(Float.toString(score)).length():4));
	...
	public void newRandom(){
		if(walls.size() > 30)return;
		
		Rectangle intersection = new Rectangle();
		
		float randomx = randomizer.nextFloat() * (6f - 0.5f) + 0.5f;
		float randomy = randomizer.nextFloat() * ( 	(player.getPosition().y + (Game.V_HEIGHT/100))
		...
		if(walls.size() == 0){
			...
			return;
					...
					for(Walls wall : walls){
						...
						if(!Intersector.intersectRectangles(tmpborder,wall.border,intersection) && wall == walls.get(walls.size()-1)){
							toadd.add(new Vector2(randomx,randomy));
							return;
						}
					}
	...
	public void dispose() {}
	public boolean debugon(){return debug = true;}
	public boolean debugoff(){return debug = false;}

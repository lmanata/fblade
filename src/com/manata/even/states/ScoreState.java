...
public class ScoreState extends GameState {
	...
	public void handleInput() {
		...
		if (Gdx.input.isKeyJustPressed(Input.Keys.RIGHT) && currentItem == 0)
			currentItem = 1;

		if (Gdx.input.isKeyJustPressed(Input.Keys.LEFT) && currentItem == 0)
			currentItem = 1;
	...
	public void render() {
		...
		for (int i = 0; i < menuItems.length; i++) {
			...
			if (i == 0) {
				...
				font.draw(sb, layout, (Game.V_WIDTH - layout.width) / 3 - 15, Game.V_HEIGHT / 2);
				cnt++;
				continue;
			}
			layout.setText(font, menuItems[i]);
